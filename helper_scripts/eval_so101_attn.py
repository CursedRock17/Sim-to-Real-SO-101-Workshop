...
"""
SO-101 Real-Robot GR00T Policy Evaluation with Attention Visualization.

Companion client for run_gr00t_server_attn.py. Same eval loop as
eval_so101_merged.py, but configured to connect to the attention-enabled
server which streams real-time attention overlays to rerun.

The attention visualization happens SERVER-SIDE (where the model lives).
This client just drives the robot and sends observations — set --rerun=false
here since the server handles the rerun viewer.

Two-terminal workflow:
    # Terminal 1 — attention server (GPU machine)
    python3 run_gr00t_server_attn.py \\
        --model-path /tmp/so101_groot/checkpoint-XXXX \\
        --embodiment-tag NEW_EMBODIMENT \\
        --port 5556 \\
        --camera-keys external_D455 ego \\
        --probe  # first run only

    # Terminal 2 — robot eval (this script)
    python3 eval_so101_attn.py \\
        --robot.type=so101_follower \\
        --robot.port=/dev/ttyACM0 \\
        --robot.cameras='{ \\
            "ego": {"type": "opencv", "index": 2, "width": 640, "height": 480}, \\
            "external_D455": {"type": "opencv", "index": 0, "width": 640, "height": 480}}' \\
...
@dataclass
class EvalConfig:
    """Command-line configuration for real-robot policy evaluation with attention."""

    robot: Optional[RobotConfig] = None
    policy_host: str = "localhost"
    policy_port: int = 5556       # Attention server default port
    ...
    rerun: bool = False        # Off by default — server handles attention rerun
